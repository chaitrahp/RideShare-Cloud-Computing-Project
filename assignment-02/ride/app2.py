...
def create_ride():
	...
	#if requests.post("http://users:80/api/v1/db/read",json={"table_name":"RideShare","username":content["created_by"]}).text=="1":
	name1 = requests.get("http://users:8080/api/v1/users")
	if name1 is None:
		return Response("{}", status=400, mimetype='application/json')
	name=name1.json()
	#return name
	...
	for x in l:
		if (content["created_by"]==x):
				#return user
				requests.post("http://34.197.178.71:8000/api/v1/db/write",json={"table_name":"Ride_details","created_by":content["created_by"],"timestamp":content["timestamp"],"source":int(content["source"]),"destination":int(content["destination"]),"opt":"2"})
			#return "201"
...
def list_ride():
	...
	b = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","source":source,"destination":destination,"opt":"2"}).text=="1"

	if b:
		#compare here for timestamp,, only if the timestamp is > system time return details.
		c = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","source":source,"destination":destination,"opt":"1"})
		if c.text=="0":
...
def join_ride(ride_id):
	...
	for x in l:
		if (content["username"]==x):	
			a = True
	b = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","ride_id":ride_id,"opt":"3"}).text=="1"
	#q  = Ride.query.filter_by(ride_id=ride_id).first()
	q = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","ride_id":ride_id,"opt":"4"})
	
	if a and b and not(content["username"]==q.text):
		d = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Pool_details","ride_id":ride_id,"username":content["username"],"opt":"2"}).text=="1"
		if d:
			#return "hey"
			requests.post("http://34.197.178.71:8000/api/v1/db/write",json={"table_name":"Pool_details","ride_id":ride_id,"username":content["username"],"opt":"2"})
			#return "200"
...
def del_ride(ride_id):
	...
	b = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","ride_id":ride_id,"opt":"3"}).text=="1"
	if b:
		a = requests.post("http://34.197.178.71:8000/api/v1/db/write",json={"table_name":"Ride_details","ride_id":ride_id,"opt":"1"})
		c = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Pool_details","ride_id":ride_id,"opt":"1"}).text=="1"
		if c:
			d = requests.post("http://34.197.178.71:8000/api/v1/db/write",json={"table_name":"Pool_details","ride_id":ride_id,"opt":"1"})
...
def get_ride(ride_id):
	...
	if b:
		...
		rows = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Pool_details","ride_id":ride_id,"opt":"3"})
		#rows.text is a list of users.
		#return rows.json()
		'''for r in rows:
			u.append(r.username)'''
		#entry = Ride.query.filter_by(ride_id=ride_id).first()
		entry = requests.post("http://34.197.178.71:8000/api/v1/db/read",json={"table_name":"Ride_details","ride_id":ride_id,"opt":"5"})
		l=[]
		#return entry.text
		l=entry.json()
		result ={}
