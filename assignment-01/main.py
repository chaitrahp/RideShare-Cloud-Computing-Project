...
def create_ride():
	...
	if requests.post("http://0.0.0.0:80/api/v1/db/read",json={"table_name":"RideShare","username":content["created_by"]}).text=="1":
		requests.post("http://0.0.0.0:80/api/v1/db/write",json={"table_name":"Ride_details","created_by":content["created_by"],"timestamp":content["timestamp"],"source":int(content["source"]),"destination":int(content["destination"]),"opt":"2"})
		return Response("{}", status=201, mimetype='application/json')
	else:
		#return "400"
		return Response("{}", status=400, mimetype='application/json')
