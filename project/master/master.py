...
#to sync the slave nodes with master	
...
def clear_sync(body):
	...
	channel.basic_publish(exchange='sync_clear', routing_key='', body=body)
	connection.close()
	
#clearing the database	
def clear(ch, method, props, body):
	...
	for x in a:
		...
		session.commit()
	session.close()
	ch.basic_publish(exchange='', routing_key = props.reply_to,properties=pika.BasicProperties(correlation_id=props.correlation_id,delivery_mode=2),body="1")
	clear_sync(body)
	ch.stop_consuming()
...
#channels listening and queue initialization
if __name__=='__main__':
	...
	write_channel.queue_declare(queue='clear_queue',durable=True)
	channel1 = pika.BlockingConnection(pika.ConnectionParameters(host='rabbitmq',heartbeat=600,blocked_connection_timeout=300))
	channel = channel1.channel()
	channel.queue_declare(queue='update_queue',durable=True)
	channel.basic_qos(prefetch_count=1)
	while(1):
		method_frame, header_frame, body = write_channel.basic_get('write_queue')
		if method_frame:
			#print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Message is present ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
			write(write_channel,method_frame, header_frame, body)

		method_frame1, header_frame1, body1 = write_channel.basic_get('clear_queue')
		if mf1:
			#print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Message is present ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
			clear(write_channel,method_frame1, header_frame1, body1)
		
		method_frame2, header_frame2, body2 = channel.basic_get('update_queue')
		if mf2:
			#print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Message is present ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
			update(channel,method_frame2, header_frame2, body2)
